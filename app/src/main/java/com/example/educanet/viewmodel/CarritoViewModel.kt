...
class CarritoViewModel : ViewModel() {
    ...
    fun agregarAlCarrito(libro: Libro) {
        viewModelScope.launch {
            try {
                ...
                if (libroActualizado == null || libroActualizado.stock <= 0) return@launch

                val carritoActual = _carrito.value.toMutableList()
                val itemExistente = carritoActual.find { it.libro.id == libro.id }

                // Verificar stock disponible
                val stockDisponible = libroActualizado.stock - (itemExistente?.cantidad ?: 0)
                if (stockDisponible <= 0) return@launch

                if (itemExistente != null) {
                    itemExistente.cantidad++
                    // Actualizar stock en Firestore
                    repository.actualizarStock(libro.id, libroActualizado.stock - 1)
                } else {
                ...
                if (itemExistente != null) {
                    ...
                    if (libroActualizado != null) {
                        repository.actualizarStock(
                            libro.id,
                            libroActualizado.stock + itemExistente.cantidad
                        )
                    }
                }
            ...
            try {
                ...
                _carrito.value.forEach { item ->
                    ...
                    if (libroActualizado != null) {
                        repository.actualizarStock(
                            item.libro.id,
                            libroActualizado.stock + item.cantidad
                        )
                    }
                }
    ...
    //fun obtenerTotal(): Double {
        //return _carrito.value.sumOf { it.libro.precio * it.cantidad }
    //}
}